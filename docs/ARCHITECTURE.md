...
This document describes the design model of `rhio` and the architectural decisions that have been **locked** — settled during initial design and not up for reopening without explicit signal.

## The model: block-based co-residency

A `rhio`-managed file is **not** a symlink, a generated artifact, or a template render. It is a real file owned by the user. Inside, a delimited block is owned by `rhio`:

```
<user content above>

# >>> RhuffusIO Managed Block >>>
<rhio-managed content>
# <<< RhuffusIO Managed Block <<<
...
1. The user's personal edits coexist with rhio's managed content in the same file.
2. Drift detection is well-defined: content inside the markers belongs to rhio, outside belongs to the user.
3. Removal is non-destructive: `rhio remove` strips only the block, preserving everything else.

## Sidecar state: `.rhio`

Each managed file at `<path>` has a sibling sidecar at `<path>.rhio` (e.g., `~/.zshrc.rhio`). It is TOML and intended to be versioned in git for portability across machines.
...
- **What rhio expects this file's block to contain** (via `managed_block_hash`) — used for drift detection.
...
`chezmoi` uses a central BoltDB at `~/.config/chezmoi/chezmoistate.boltdb`. `rhio` instead uses a per-file sidecar because:
...
To decide whether two stanzas are "the same configuration" with different syntax (e.g., `FOO=bar` outside the block vs `export FOO=bar` inside), `rhio` parses each side and compares AST nodes — not strings.
...
- **API key management** chicken-and-egg: rhio's purpose is to manage secrets, but the LLM API needs a secret.
...
Tree-sitter is deterministic, offline, sub-millisecond, free, and well-supported. If a comparison is ambiguous, `rhio` asks the user — not an LLM.

## Build vs adopt: why not `chezmoi` or `yadm`?

These were evaluated against `rhio`'s scope:
...
4. `.rhio` sidecar per file persisting user decisions.
...
| Binary distribution | Homebrew tap `rhuffus/rhio` |
| Comparison engine | tree-sitter only (no LLM) |
| State storage | per-file `.rhio` sidecar, TOML, versioned in git |
...
| License | Apache-2.0 |
| Naming | `rhio` (acronym RhuffusIO); same name for binary, repo, crate, tap |
